...
@Controller
public class UERegInfoController_GPSTIME {
	private static final Logger logger = Logger.getLogger(UERegInfoController_GPSTIME.class);
	
	@RequestMapping("/ueRegInfo_gpstime")
	public ModelAndView ueRegInfo(HttpServletRequest request) {
		final String userIpAddress = request.getRemoteAddr();
		final String userAgent = request.getHeader("user-agent");
		logger.info("RemoteAddress : " + userIpAddress);
		logger.info("Controller : Get Data order by GPSTIME");
		logger.info("Header : " + userAgent);
		
		final int index = UERegInfo.INDEX_GPSTIME;
