...
@Controller
public class UERegInfoController_Time {
	private static final Logger logger = Logger.getLogger(UERegInfoController_Time.class);
	
	@RequestMapping("/ueRegInfo_time")
	public ModelAndView ueRegInfo(HttpServletRequest request) {
		final String userIpAddress = request.getRemoteAddr();
		final String userAgent = request.getHeader("user-agent");
		logger.info("RemoteAddress : " + userIpAddress);
		logger.info("Controller : Get Data order by TIME");
		logger.info("Header : " + userAgent);
		
		final int index = UERegInfo.INDEX_TIME;
