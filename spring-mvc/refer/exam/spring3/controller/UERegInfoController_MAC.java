...
@Controller
public class UERegInfoController_MAC {
	private static final Logger logger = Logger.getLogger(UERegInfoController_MAC.class);
	
	@RequestMapping("/ueRegInfo_mac")
	public ModelAndView ueRegInfo(HttpServletRequest request) {
		final String userIpAddress = request.getRemoteAddr();
		final String userAgent = request.getHeader("user-agent");
		logger.info("RemoteAddress : " + userIpAddress);
		logger.info("Controller : Get Data order by MAC");
		logger.info("Header : " + userAgent);
		
		final int index = UERegInfo.INDEX_MAC;
