...
@Controller
public class UERegInfoController_RIP {
	private static final Logger logger = Logger.getLogger(UERegInfoController_RIP.class);
	
	@RequestMapping("/ueRegInfo_rip")
	public ModelAndView ueRegInfo(HttpServletRequest request) {
		final String userIpAddress = request.getRemoteAddr();
		final String userAgent = request.getHeader("user-agent");
		logger.info("RemoteAddress : " + userIpAddress);
		logger.info("Controller : Get Data order by RIP");
		logger.info("Header : " + userAgent);
		
		final int index = UERegInfo.INDEX_RIP;
