...
@Controller
public class UERegInfoController_ESN {
	private static final Logger logger = Logger.getLogger(UERegInfoController_ESN.class);
	
	@RequestMapping("/ueRegInfo_esn")
	public ModelAndView ueRegInfo(HttpServletRequest request) {
		final String userIpAddress = request.getRemoteAddr();
		final String userAgent = request.getHeader("user-agent");
		logger.info("RemoteAddress : " + userIpAddress);
		logger.info("Controller : Get Data order by ESN");
		logger.info("Header : " + userAgent);
		
		final int index = UERegInfo.INDEX_ESN;
